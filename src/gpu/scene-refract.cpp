    ...
    priv_(0)
{
    ...
    options_["model"] = Scene::Option("model", "arvind", "Which model to use",
                                      optionValues);
    optionValues = "";
...
DistanceRenderTarget::setup(unsigned int canvas_fbo, unsigned int width, unsigned int height)
{
    ...
    width_ = canvas_width_ * GPULOAD;
    height_ = canvas_height_ * GPULOAD;
...
RefractPrivate::draw()
{
    ...
    modelview_.rotate(rotation_, 0.0f, 1.0f, 0.0f);   // Arvind comment this line if you want to stop rotation
    if (orientModel_)
    {
        modelview_.rotate(orientationAngle_, orientationVec_.x(), orientationVec_.y(), orientationVec_.z());
    }
