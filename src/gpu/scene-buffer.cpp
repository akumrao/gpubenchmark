/*
 *  Class for wallpaper and tiles buffer comparison
 *
 */
...
//    DIR *dr;
//    struct dirent *en;
//    dr = opendir(storagePath.c_str()); //open all or present directory
//    if (dr) {
//        while ((en = readdir(dr)) != NULL) {
//            // printf("%s\n", en->d_name); //print all directory name
//            if(en-> d_type == DT_REG && strcmp(en->d_name,".")!=0 && strcmp(en->d_name,"..")!=0 ) // if it is a directory
//            {
//                int sz = strlen(en->d_name);
//                if( !strncmp(  &en->d_name[sz-4], ".rgb",4  ) &&  strncmp(  en->d_name, "buffer", 6  )) {
//
//                    std::string  tmp =  storagePath  + en->d_name;
//                    stRgbInf rgbInf;
//                    rgbInf.file = tmp;
//                    rgbInf.w = 1915;
//                    rgbInf.h = 735;
...
void SceneBuffer::readfile(std::string file, int w, int h, int pos)
{

    if(!m_rgb)
    {
        int sz = clipstorage.size();
        m_rgb = new unsigned char[w * h * 4 * sz];
    }
    ...
    if (readPd.is_open()) {
        ...
        //   const char *pixels1 = tmp.c_str();

        memcpy(&m_rgb[pos], strStream.str().c_str(), w * h * 4);
...
void SceneBuffer::loadtx(std::string name, GLuint *tex,  GLint min_filter, GLint mag_filter)
{
   ...
   // 1915 * 735
    readfile(rgbfl.file, rgbfl.w, rgbfl.h, 0);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, format, rgbfl.w, rgbfl.h, 0,
                 format, GL_UNSIGNED_BYTE, m_rgb);



}

void SceneBuffer::loadmultitx(std::string name, GLuint *tex,  GLint min_filter, GLint mag_filter)
{
    ...
    std::vector<stRgbInf>::iterator iitStore = clipstorage.begin();
    int count=0;
    while( iitStore != clipstorage.end()) {
        stRgbInf rgbfl = *iitStore;;
        readfile(rgbfl.file, rgbfl.w, rgbfl.h, count* rgbfl.w * rgbfl.h*4);
        w = rgbfl.w;
        h = rgbfl.h;
    ...
    // if (needs_mipmap && !GLExtensions::GenerateMipmap)
    // glTexParameteri(GL_TEXTURE_2D, GL_GENERATE_MIPMAP, GL_TRUE);
    glTexImage2D(GL_TEXTURE_2D, 0, format, w, h*count, 0,
                 format, GL_UNSIGNED_BYTE, m_rgb);
...
SceneBuffer::runit()
{
    ...
    if(done)
        loadmultitx("effect-2d", &texture_,
                    GL_NEAREST, GL_NEAREST);
    else
        loadtx("effect-2d", &texture_,
               GL_NEAREST, GL_NEAREST);
...
SceneBuffer::draw()
{
    ...
    if( !done && !nCount &&  itStore  != clipstorage.end()){
    ...
    if( !done && itStore  == clipstorage.end()){
        done = true;
        runit();
    }
