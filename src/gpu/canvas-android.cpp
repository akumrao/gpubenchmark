...
*/
   ...
   EGLint attribs[] = {
    ...
    eglQueryContext(egl_display, egl_context, EGL_CONFIG_ID, &(attribs[1])); // arvind remove it if you want to change the color depth

    eglChooseConfig(egl_display, attribs, &egl_config, 1, &num_configs);

    /* Before calling GLES functions, init GLAD GLES */
    if (!gles_lib_.open("libGLESv3.so")) {
...
void
CanvasAndroid::read_pixelbuf(char *pixels)
{
...
//    for (int i = 0; i < height_; i++) {
//        glReadPixels(0, 0, width_, 1, GL_RGBA, GL_UNSIGNED_BYTE,
//                     &pixels[(height_ - i - 1) * width_ * 4]);
//    }
